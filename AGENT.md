...
UI 必须如实区分 loading / 空 / 不可用 / 降级 / 失败 / 取消 / 完成。不得把启发式、
provider 字幕、推测或模型输出，呈现得比后端 provenance 所支持的更确定。
测试不为了绿而绿，报告不报喜。

一个教语言的产品，一旦在证据强度上撒谎，就没有价值了。
...
- `design-notes/listen-design-charter.md` — 设计宪章，五条可检验原则，discipline 测试引用它
