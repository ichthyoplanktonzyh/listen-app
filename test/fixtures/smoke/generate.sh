...
out="$root/generated"
ffmpeg="$root/../../../.backend/runtime/runtime/ffmpeg"
[[ -x "$ffmpeg" ]] || {
  echo "Install the locked listen-core runtime before generating smoke fixtures." >&2
  exit 1
}
mkdir -p "$out"
...
"$ffmpeg" -hide_banner -loglevel error -y \
  ...
  -c:v mpeg4 -q:v 5 -pix_fmt yuv420p -c:a aac -shortest "$out/sample-video.mp4"
...
"$ffmpeg" -hide_banner -loglevel error -y \
  ...
  -metadata:s:a:1 language=jpn -c:v mpeg4 -q:v 5 -pix_fmt yuv420p -c:a aac \
