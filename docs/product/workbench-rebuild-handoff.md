...
| 精听态形态 | 逻辑全复用，**浮动窗 → 全覆盖专注壳**（顶部 `N/总` + `倍速/次数/间隔`、中间提示区、底部键盘提示条） |
...
- [ ] **P0-d 返回当前句改 FAB** ← **下一步**（会改动 `transcript_panel_test.dart` 中钉住「底部条」`_BackToCurrentBar` 的三个测试，见 §6）。

**P1**：解析面板 `文字/语音` 切换 · 精听态全覆盖专注壳（复用 `PracticeController`/`PracticeActionsCoordinator`）· 底栏三段式 + 媒体缩略图/标题 + 倍速药丸 · 导出/打印（纯前端可做部分）+ 分享入口。

**P2**：播放列表（queue model，后端缺口）· 桌面浮层字幕 · 词典气泡丰富度 · 全屏 · 设置分区（桌面字幕 tab）。

**后端缺口台账**：`docs/product/workbench-backend-gaps.md`（P0-b 已建）——已记：媒体来源层级/双语标题（面包屑要用）、导出/打印/离线、分享、跟读评分绑定 cue；待记：次数/间隔、听写先选句、文字层面 AI 解析契约、解析按 cue 缓存、播放列表 queue。（选词数据**不是缺口**，已从台账删除——见台账 §4 前的修正注。）逐条随 P1 消账。
...
- 精听逻辑（**好资产，全复用**）：`controllers/practice_controller.dart`、`controllers/practice_actions_coordinator.dart`、`widgets/panels/intensive_practice_window.dart`（884 行，当前是**可拖拽浮动窗** `Positioned` in Stack ≤860×560 → 要改成全覆盖壳）。
...
## 5. 下一步 P0-d 返回当前句改浮动 FAB —— 落地要点

**先看真图 `~/Desktop/每日英语听力UI/浮动重新定位.png`**（右下角双箭头浮动按钮）。目标：把文稿底部的 `_BackToCurrentBar` 横条换成右下角浮动 FAB。
- 现状：`transcript_panel.dart` 里 `_BackToCurrentBar`，在 `_readThroughList(context)` 的 Column 底部、`!_following && currentCue != null` 时显示。**注意 P0-c 重构后文稿列表逻辑已抽到 `_readThroughList` 方法**（盲听/选词是单句聚焦视图、无滚动列表，FAB 只属于 normal 通读态）。
- FAB 要点（gap-analysis §3.8 + 决定表）：右下角浮动；仅滚离后（`!_following`）出现；**避让当前句**（别盖住右下角那一句——参考的做法/或收到列表内边距）；tooltip + 语义标签；窄窗可用。
- 形态从「让出等高一条」变成「浮在列表上」，所以要处理遮挡：可在 FAB 显示时给 ListView 底部留 padding，或让 FAB 半透明/避让。
- **必改测试**（见 §6）：`test/transcript_panel_test.dart` 里钉 `_BackToCurrentBar` 的两个测试。

（P0-b/P0-c 的落地要点已完成，删去；如需回看实现改动见 §3 各条列的文件。）

## 6. 别再踩的坑

- **P0-d 改 FAB 时**：`test/transcript_panel_test.dart` 有两个测试钉住「底部条」——`manual scroll pauses following and back button resumes it`（`回到当前句` findsOneWidget/Nothing、tap 恢复）、`back-to-current sits under the list and covers no sentence`（`Key('transcript-back-to-current')` 在 list 下方、让出等高空间）。改 FAB 必须同步改这两个并写清理由（规则由测试执行）。
